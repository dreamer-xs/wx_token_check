...
void do_verify(int fd,char *fileName)
{
...
Context-Length:  ";
    ...
    char echostr[100] = {0};
    char echostrValue[1024] = {0};
    char timestamp[100] = {0};
    char nonce[100] = {0};
    sscanf(fileName,"%[^&]&%[^&]&%[^&]&%[^&]",signature,echostr,timestamp, nonce);
    printf("%s\n", echostr);
    sscanf(echostr, "%[^=]=%[^=]", signature, echostrValue);
    printf("%s\n", echostrValue);

    char data[1024] = {0};
    //http头和数据中间必须要有换行
    sprintf(data, "%lu\r\n\r\n%s\r\n", strlen(echostrValue), echostrValue);

    printf("-----httpHeader----%lu----\n",strlen(httpHeader));
    printf("%s", httpHeader);
    printf("%s", data);
    printf("--------data------%lu-----\n",strlen(data));

    send(fd,httpHeader,strlen(httpHeader),0);
...
//		shmctl(shmid,IPC_RMID,NULL);
	...
	while(1)
	{
		...
		{
            //判断是否校验token
            if(strstr(fileName, "echostr") != NULL)
                do_verify(clientfd,fileName);
            else
